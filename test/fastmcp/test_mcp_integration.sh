...
echo "LOAD 'duckdb_mcp'; ATTACH 'stdio://$SERVER_CMD' AS testdata (TYPE mcp); SHOW DATABASES;" | build/release/duckdb

# Test file access through MCPFS
echo "3. Testing file access through MCPFS..."
echo "LOAD 'duckdb_mcp'; ATTACH 'stdio://$SERVER_CMD' AS testdata (TYPE mcp); SELECT * FROM read_csv('mcp://testdata/file:///customers.csv');" | build/release/duckdb
