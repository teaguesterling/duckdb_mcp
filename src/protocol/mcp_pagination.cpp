...
MCPPaginationResult MCPPaginationIterator::Next() {
    ...
    try {
        ...
        string items_field;
        if (method_name == "resources/list") items_field = "resources";
        else if (method_name == "prompts/list") items_field = "prompts";  
        else if (method_name == "tools/list") items_field = "tools";
        else throw InvalidInputException("Unsupported pagination method: " + method_name);
        ...
        MCP_LOG_DEBUG("PAGINATION", "Fetched page with %llu items, next_cursor: %s", 
                     current_result.items.size(), current_result.next_cursor.c_str());
