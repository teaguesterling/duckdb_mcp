...
MCPMessage MCPServer::HandleResourcesList(const MCPMessage &request) {
	...
	for (const auto &uri : resource_uris) {
		...
		if (provider) {
			Value resource = Value::STRUCT({{"uri", Value(uri)},
	...
	Value result = Value::STRUCT({{"resources", resources_list}});
...
MCPMessage MCPServer::HandleResourcesRead(const MCPMessage &request) {
	...
	Value contents_item = Value::STRUCT(
	    {{"uri", Value(uri)}, {"mimeType", Value(read_result.mime_type)}, {"text", Value(read_result.content)}});

	Value result = Value::STRUCT({{"contents", Value::LIST(contents_struct_type, {contents_item})}});
...
MCPMessage MCPServer::HandleToolsList(const MCPMessage &request) {
	...
	Value result = Value::STRUCT({{"tools", tools_list}});
