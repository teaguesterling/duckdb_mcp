    ...
    auto mcp_handler = [&config, &request_handler](const CPPHTTPLIB_NAMESPACE::Request &req,
        ...
        if (!config.auth_token.empty()) {
            ...
            if (auth_header.empty()) {
                ...
                res.set_content(R"({"jsonrpc":"2.0","error":{"code":-32001,"message":"Unauthorized: authentication required"},"id":null})",
                               "application/json");
                return;
            }
            ...
            if (auth_header != expected) {
                ...
                res.status = 403;
                res.set_content(R"({"jsonrpc":"2.0","error":{"code":-32003,"message":"Forbidden: invalid credentials"},"id":null})",
                               "application/json");
                return;
            }
        }
        ...
        try {
            ...
            res.set_content(R"({"jsonrpc":"2.0","error":{"code":-32603,"message":"Internal error: )" +
                           string(e.what()) + R"("},"id":null})", "application/json");
        }
    };
