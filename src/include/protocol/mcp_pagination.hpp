...
struct MCPPaginationResult {
    ...
    bool HasNextPage() const { return has_more_pages && !next_cursor.empty(); }
    
    // Get the cursor for the next page request
    string GetNextCursor() const { return next_cursor; }
...
struct MCPPaginationParams {
    ...
    MCPPaginationParams(const string &cursor, idx_t limit = 0) 
        : cursor(cursor), limit(limit) {}
    
    // Check if this is the first page request
    bool IsFirstPage() const { return cursor.empty(); }
...
public:
    ...
    // Error handling
    bool IsValid() const { return !is_finished; }
...
public:
    ...
    shared_ptr<MCPConnection> GetConnection() const { return connection; }
};
