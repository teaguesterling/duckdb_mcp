...
public:
	...
	virtual bool ShouldRefresh() const {
		return false;
	}
...
public:
	QueryResourceProvider(const string &query, const string &format, DatabaseInstance &db,
	                      uint32_t refresh_interval_seconds = 0);
	...
	bool IsRefreshable() const override {
		return refresh_interval_seconds > 0;
	}
