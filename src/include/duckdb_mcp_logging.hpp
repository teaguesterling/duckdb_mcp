...
enum class MCPLogLevel : uint8_t { TRACE = 0, DEBUG = 1, INFO = 2, WARN = 3, ERROR = 4, OFF = 5 };
...
#define MCP_LOG_PROTOCOL(outgoing, server, json) MCPLogger::GetInstance().LogProtocolMessage(outgoing, server, json)
...
#define MCP_PERF_TIMER(operation, details) MCPPerformanceTimer _perf_timer(operation, details)
