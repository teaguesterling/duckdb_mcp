...
void MCPLogger::LogPerformanceMetric(const string &operation, double duration_ms, const string &details) {
    ...
    string message;
    if (!details.empty()) {
        message = StringUtil::Format("PERF: %s completed in %.2fms (%s)", 
                                   operation, duration_ms, details);
    } else {
        message = StringUtil::Format("PERF: %s completed in %.2fms", 
